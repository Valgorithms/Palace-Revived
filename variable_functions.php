...
$slash_init = function (\Tutelar\Tutelar $tutelar)
{
    ...
    $tutelar->discord->application->commands->freshen()->done(
        function ($commands) use ($tutelar) {
...
$on_ready = function (\Tutelar\Tutelar $tutelar) use ($slash_init)
{
...
$manager_message = function (\Tutelar\Tutelar $tutelar, $message, string $message_content, string $message_content_lower)
{
    ...
    if(str_starts_with($message_content_lower, 'setup')) {
        ...
        if(str_starts_with($message_content_lower, $message->guild_id)) { //Creates default channels, roles
            //
        }
